...
std::pair<Vector3, Vector3> DrawableObject::getBoundingBox() {
	Vector3 max{ -INFINITY, -INFINITY, -INFINITY };
	Vector3 min{ INFINITY, INFINITY, INFINITY };

	for (int i = 0; i < points.size(); ++i) {
		for (int v = 0; v < 3; ++v) {
			if (points[i].values[v] < min.values[v]) {
				min.values[v] = points[i].values[v];
			}
			if (points[i].values[v] > max.values[v]) {
				max.values[v] = points[i].values[v];
			}
		}
	}
